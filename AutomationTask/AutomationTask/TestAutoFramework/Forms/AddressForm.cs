...
namespace AutomationTask.TestAutoFramework.Forms
{
    public class AddressForm : Form
    {
        ...
        public void EnterAdressOrPostcode(string adressPostcodeData)
        {
            foreach (var symbol in adressPostcodeData)
            {
                ...
                AqualityServices.ConditionalWait.
                    WaitFor(() => SearchedAdress(adressPostcodeData).State.IsDisplayed == false, TimeSpan.FromSeconds(1));
            }
        ...
        {
            AqualityServices.ConditionalWait.
                      WaitFor(() => TrySearchAgain.State.IsDisplayed == false, TimeSpan.FromSeconds(3));
